...
public class Indexado {
    ...
    public void escribirArchivo(int llave, long pos) {
        try {
            RandomAccessFile archivo = new RandomAccessFile("indexV.gsh", "rw");
    ...
    public long leerArchivoSecuencial(int llave) {
        ...
        try {
            ...
            leer = new RandomAccessFile("indexV.gsh", "r");
    ...
    public long eliminarArchivoSecuencial(int llave) {
        ...
        long apActual, apFinal, salida = -1;
        try {
            leer = new RandomAccessFile("indexV.gsh", "rw");
            while ((apActual = leer.getFilePointer()) != (apFinal = leer.length())) {
                valor = leer.readInt();
                if (valor == llave) {
                    salida = leer.readLong();
    ...
    public void mostrarTodo() {
        ...
        try {
            ...
            while ((apActual = leer.getFilePointer()) != (apFinal = leer.length())) {
                ...
                auxd = leer.readLong();
                if (auxl != 0) {
                    if (auxl > 0 && (auxd / 210 + 1) != 0)
                        System.out.println("Llave: " + auxl + " - Dirección: " + (auxd / 210 + 1));
                }
            }
            leer.close();
        } catch (IOException e) {
